        ...
        <aside id="sidebar" class="sidebar bg-gradient-to-b from-[#1e3a5f] via-[#2c4f7c] to-[#1a2f4d] shadow-2xl flex flex-col border-r border-blue-800/30">
            ...
            <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto custom-scrollbar">
                ...
                <div class="pt-6">
                    ...
                    @if(auth()->check() && (auth()->user()->isSectorista() || auth()->user()->isSecretarioCTPPGE() || auth()->user()->isAdmin() || auth()->user()->isProcuradorPGE()))
                    <a href="{{ route('activity2.index', auth()->user()->sectorista_id ?? '') }}" class="nav-item {{ request()->routeIs('activity2.*') || request()->routeIs('meetings.*') || request()->routeIs('oficios.*') ? 'active' : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/>
                        </svg>
                        <span class="flex-1">Actividad 2: Órgano Colegiado</span>
                        <span class="text-[10px] bg-yellow-500/20 text-yellow-300 px-2 py-0.5 rounded-full font-medium">1d</span>
                    </a>
                    @endif
